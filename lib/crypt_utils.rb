...
class CryptUtils
  ...
  def self.one_way_crypt(password)
    Digest::SHA1.hexdigest(password)
  end

  def self.new_keypair
    keypair = PKey::RSA.new(2048)
    keypair
  end

  def self.get_private_key_from_keypair(keypair)
    keypair.to_s
  end

  def self.get_public_key_from_keypair(keypair)
    keypair.public_key.to_s
  end

  def self.decrypt_team_password(team_password, private_key)
  ...
  def self.encrypt_team_password(team_password, public_key)
  ...
  def self.validate_keypair(private_key, public_key)
    test_data = 'Test Data'
  ...
  def self.encrypt_blob(blob, team_password)
    cipher = OpenSSL::Cipher::Cipher.new(@@cypher)
    cipher.encrypt
  ...
  def self.decrypt_blob(blob, team_password)
    cipher = OpenSSL::Cipher::Cipher.new(@@cypher)
    cipher.decrypt
    cipher.key = team_password
