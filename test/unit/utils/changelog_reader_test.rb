...
class ChangelogReaderTest < ActiveSupport::TestCase
  ...
  context 'test ChangelogReader' do
    ...
    test 'header line parsed' do
      version = subject.send(:changelog_header_line, '## Version 0.0')
      assert_equal('0.0', version)
    end

    test 'entry line parsed' do
      entry = subject.send(:changelog_entry_line, '* change')
      assert_equal('change', entry)
    end

    test 'doesnt parse invalid line' do
      version = subject.send(:changelog_header_line, 'invalid')
      assert_nil(version)

      entry = subject.send(:changelog_entry_line, 'invalid')
      assert_nil(entry)
    end
  end
end
